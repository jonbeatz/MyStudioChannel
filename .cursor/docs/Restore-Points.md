...
| ID | Date | Summary |
|----|------|---------|
| **RP-2026-06-16-hermes-gateway-session** | 2026-06-16 | **Hermes Telegram gateway deferred to Start Project:** Removed Windows logon scheduled task (`hermes gateway uninstall`) so no `cmd.exe` popup at boot. `scripts/start-hermes-api.ps1` starts gateway after LiteLLM + ngrok via `hermes gateway install --no-start-on-login --start-now`. `msc:litellm:stop` / End Project stops gateway with ports 3000/4000/4040. **Branch:** `MSC-Website-v9` (uncommitted script + doc deltas). **Restore logon auto-start (not recommended):** `hermes gateway install`. |
