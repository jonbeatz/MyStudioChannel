...
| ID | Date | Summary |
|----|------|---------|
| **RP-2026-06-13-ngrok-restored** | 2026-06-13 | **LiteLLM + ngrok Start Project Integration:** Restored the `start-hermes-api.ps1` launcher to boot both LiteLLM (via elevated Windows Terminal) and ngrok (via minimized background process to prevent double-UAC prompts). Added automated inspection of the `4040` local ngrok API, validation of remote model accessibility via public `/v1/models`, dynamic `.cursor/session-google-api.json` generation, and Cursor settings logging. All tests and completions pass. **Branch/commit:** `MSC-Website-v8` @ `HEAD`. **Restore:** `git fetch origin && git checkout MSC-Website-v8 && git reset --hard HEAD`. |
