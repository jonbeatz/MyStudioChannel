...
| **`npm run push:website:live`** | **Unified deploy ritual:** **`verify:next:safe`** → **`verify:ftp-smoke`** → server-prep instructions → **`pushit:live`** → poll **`verify:live`** → **`verify:live:version`**. Say *“push website live”* / *“push it live”* / *“deploy live”* in chat. Dry-run: **`npm run push:website:live -- --dry-run`**. Deploy zips: **`zips/MyStudioChannel-v4-deploy-YYYYMMDD-HHmmss.zip`**. |
...
Default workflow: **`npm run push:website:live`** (preflight + build + FTPS + live/version verify) or **`npm run pushit:live`** (upload only) then restart Node in hPanel. After Tier 2, run **`npm run dev`** or **`dev:fresh`** locally when you want **localhost** again.  
