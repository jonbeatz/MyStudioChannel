...
**Release version check:** After deploy, open **`/admin`** — sidebar (bottom-left) should show **`MyStudioChannel Admin v4.0.0`** (matches root **`package.json`** **`"version"`**). Bump **`package.json`** only when you cut a new release (e.g. `4.0.0` → `4.0.1`); UI labels read from **`lib/msc-app-version.ts`** at build time.
...
**Single source of truth:** root **`package.json`** **`"version"`** (currently **`4.0.0`** on **`MSC-Website-v4`**). **`lib/msc-app-version.ts`** imports it for the marketing footer (**`MyStudioChannel v4.0.0`**) and Payload admin sidebar (**`MyStudioChannel Admin v4.0.0`**).
...
| **`npm run push:website:live`** | **Default (MCP zip):** kill dev → **`npm run build`** → **`deploy:zip`** → agent **`hosting_deployJsApplication`** → hPanel **restart** → **`verify:live`**. Say *“push website live”* in chat. **FTPS fallback:** **`npm run push:website:live -- --ftps`**. Dry-run: **`-- --dry-run`**. Zips: **`zips/MyStudioChannel-deploy-YYYYMMDD-HHmmss.zip`**. Restart: https://hpanel.hostinger.com/websites/mystudiochannel.com |
...
Default workflow: **`npm run push:website:live`** (MCP zip) or **`-- --ftps`** if MCP fails; legacy **`npm run pushit:live`** (FTPS only). Always **restart Node** in hPanel after deploy. After deploy, run **`npm run dev`** or **`dev:fresh`** locally when you want **localhost** again.  
